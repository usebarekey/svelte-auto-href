...
export function get_href_completions(
	...
	return [...completions.values()].sort((left, right) => left.label.localeCompare(right.label));
}
 ...
 */
export function is_known_app_href(manifest: AutoHrefManifest, href: string): boolean {
	...
	return manifest.routes.some((route) => route_matches_pathname(route, pathname));
}
...
function route_matches_pathname(route: RouteRecord, pathname: string): boolean {
	...
	return route.templates.some((template) => template_matches_pathname(route, template, pathname));
}
...
function template_matches_pathname(
	...
	const pattern = template.split("${string}").map(escape_regex).join(wildcard);
